...
const backsoundButton = document.getElementById('backsound');


// ゲーム中の音楽の初期状態
let isBacksoundOn = true;
//複数のテキストを格納する配列
const textLists = [

    // 'JKL+', 'FDSA', 'UIOP', 'TREW', 'NM<>', 'VCXZ', 
    // 'YUIO', 'BNM<', 'REWQ', 'IOP`', 'M<>?', '<>?_'
...
const keyPress = e => {
    ...
    if (e.key !== untyped.substring(0, 1)) {
        ...
        const audio = new Audio('mistype.mp3');
        audio.play();
        mistypedCount++;
...
start.addEventListener('click', () => {
    ...
    // 「スタート」ボタンを非表示にする
    start.style.display = 'none';
    backsound.style.display = '';
    // キーボードのイベント処理
    document.addEventListener('keypress', keyPress);
});

untypedfield.textContent = '毎日やれば必ず上達します!!';
// ゲームスタート前にゲーム中の音楽ONとOFFのボタンを表示
backsoundButton.style.display = 'block';
// バックサウンドのON/OFF切り替え
const backMusic = new Audio('WANIMA - ともに (OFFICIAL VIDEO).mp3');
backMusic.volume = 0.5;
backsoundButton.addEventListener('click', () => {
    if (isBacksoundOn) {
        backMusic.pause();
        isBacksoundOn = false;
        backsoundButton.textContent = 'BACKSOUND ON';
    } else {
        backMusic.play();
        isBacksoundOn = true;
        backsoundButton.textContent = 'BACKSOUND OFF';
    }
});